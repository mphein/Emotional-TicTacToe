...
class UserWebcamPlayer:
    ...
    def _get_emotion(self, img):
        ...
        print(img)
        print(image_size)
        image = np.reshape(img, image_size)
        ...
        model = keras.models.load_model('best_model.keras').predict(image)
        print(model)
        # emotions = []
        ...
        #     emotions.append(num)
        # print(emotions)
        # print(emotions.index(max(emotions)))
        return 0
        return emotions.index(max(emotions))
